...
~/.claude/meeting-summarize/        # 환경변수 MEETING_SUMMARIZE_HOME 으로 위치 변경
  context.md     우리 조직·구성원·사업·거래처 — 직접 편집하거나 "새 멤버 추가해 줘"
  feedback.md    리뷰 피드백에서 나온 규칙(F-NN) — 자동 누적
...
회의록을 `_v2`로 고치고, 일반 규칙은 데이터 폴더의 `feedback.md`에 `F-NN` 항목으로 추가합니다. 이후 모든 회의록은 스킬의 기본 규칙(`references/rules.md`, R-01~R-05)과 이 로그를 함께 대조한 뒤 나오며, 둘이 충돌하면 사용자의 `F-NN`이 우선합니다. 스킬 폴더는 읽기 전용으로 취급하므로 플러그인 업데이트로 규칙이 사라지지 않습니다.
